...
cmd:option('-trainingSetLMDBPath', './prepare_datasets/text_lmdb/train/', 'Path to LMDB training dataset')
cmd:option('-validationSetLMDBPath', './prepare_datasets/text_lmdb/test/', 'Path to LMDB test dataset')
...
cmd:option('-testAfterEachEpoch', false, 'Whether run testNetwork after each epoch')
...
cmd:option('-dictionaryPath', './dictionary_text', ' File containing the dictionary to use')
cmd:option('-epochs', 30, 'Number of epochs for training')
cmd:option('-learningRate', 3e-4, ' Training learning rate')
cmd:option('-learningRateAnnealing', 1.05, 'Factor to anneal lr every epoch')
...
local net_opt = json.load('params_text.json')
